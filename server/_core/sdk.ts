...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    const decodedSessionCookie = sessionCookie ? decodeURIComponent(sessionCookie) : undefined;

    if (!sessionCookie && !decodedSessionCookie) {
      throw ForbiddenError("Invalid session cookie");
    }

    // Try to parse as local session (JSON)
    try {
      const sessionData = JSON.parse(decodedSessionCookie || sessionCookie || "");
