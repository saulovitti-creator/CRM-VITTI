...
export function TodayTasks() {
  ...
  const coldOpportunities = alertsQuery.data || [];

  const totalItems = todayTasks.length + overdueTasks.length + coldOpportunities.length;
      ...
      >
        <CardContent>
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
            {coldOpportunities.map((opportunity: any) => {
              const daysSince = opportunity.lastContactAt
                ? Math.floor((Date.now() - new Date(opportunity.lastContactAt).getTime()) / (1000 * 60 * 60 * 24))
                : null;

              return (
                <div key={`opportunity-${opportunity.id}`} className="group flex items-center justify-between p-3 rounded-lg bg-red-900/20 border border-red-500/40 hover:bg-red-900/30 transition-colors">
                  <div className="flex flex-col gap-1">
                    <p className="text-red-400 font-bold text-sm flex items-center gap-1">
                      <Flame className="w-4 h-4" /> URGENTE: Ligar para {opportunity.companyName}
                    </p>
                    <p className="text-xs text-red-300/80 mt-1">
                      Oportunidade parada em "{opportunity.status}"
                      {daysSince !== null ? ` ha ${daysSince} dias sem follow-up.` : " sem contato."}
                    </p>
                  </div>
                </div>
              );
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1 text-xs text-red-500/80 mt-1">
                      ...
                      Atrasada
                      {task.source === "auto" && <span className="ml-1 text-yellow-400/70">- Automatica</span>}
                    </div>
                  </div>
                </div>
              </div>
